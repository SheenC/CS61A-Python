...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        if symbol in self.bindings:
            ...
            raise SchemeError('Wooooo~ Cannot find the symbol!')
        # END PROBLEM 3
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('WOOOOOOO~ Vals cannot be matched with formals!')
        while(formals != nil):
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        new_args = operands.map(lambda x: scheme_eval(x, env))
        return scheme_apply(self, new_args, env)
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('WOOOOO Incorrect Type')
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(head):
        ...
        # BEGIN PROBLEM 6
        env.define(head, scheme_eval(expressions.second.first, env))
        return head
        # END PROBLEM 6
    elif isinstance(head, Pair) and scheme_symbolp(head.first):
        # BEGIN PROBLEM 10
        fn_name, formals = head.first, head.second
        body = expressions.second
        env.define(fn_name, LambdaProcedure(formals, body, env))
        return fn_name
        # END PROBLEM 10
    else:
        bad_head = head.first if isinstance(head, Pair) else head
        raise SchemeError('non-symbol: {0}'.format(bad_head))
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        #while loooop
        bf = bindings.first
        check_form(bf, 2, 2) #check
        vaal = scheme_eval(bf.second.first, env)
        formals = Pair(bf.first, formals)
        check_formals(formals) #check
        vals = Pair(vaal, vals)
        #while loooop
...
def do_define_macro(expressions, env):
    ...
    check_formals(head)
    check_form(head, 0)
    macro_name, formals = head.first, head.second
    body = expressions.second
...
# The last sub-expression in a tail context and, or, or begin
