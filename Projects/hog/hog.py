...
def is_swap(player_score, opponent_score):
    ...
    def seclast(x):
        return (x // 10) % 10
    def OTabs(x):
        return abs(last(x)-seclast(x))
    if OTabs(player_score) == OTabs(opponent_score):
       return True
    return False
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
          ...
          if score0 < goal and score1 < goal:
             stategy_1 = strategy1(score1, score0)
             score1 += take_turn(stategy_1, score0, dice)
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def f(score0, score1):
        ...
        if wh == 0:
           if high < (score0 - score):
              high = (score0 - score)
              k += 1
           score = score0

        if wh == 1:
           if high < (score1 - score):
              high = (score1 - score)
              k += 1
           score = score1
        
        if k > 0:
            print(high, "point(s)! That's the biggest gain yet for Player", wh)

        return announce_highest(wh, high , score)
...
def make_averaged(fn, num_samples=1000):
    ...
    def c(*args):
        ...
        while k < num_samples:
              result += fn(*args)
              k += 1
        make__average = result/num_samples

        return make__average
    return c
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    aver = 0
    i = 1
    make_averaged_rolldice = make_averaged(roll_dice)
    while i <= 10:
          current_value = make_averaged_rolldice(i, dice)
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    """
    # BEGIN PROBLEM 11
    score += take_turn(0, opponent_score, dice=six_sided)
