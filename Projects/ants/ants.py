...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        # BEGIN EC
        "*** YOUR CODE HERE ***"
        destination = self.place.exit if self.is_exiting else self.place.entrance
        if destination is colony.hive:
            destination = None
        # END EC
        if self.blocked():
    ...
    def nearest_bee(self, hive):
        ...
        while place:
            if place.bees is hive.bees:
                break
            elif place.bees and distance >= self.min_range and distance <= self.max_range:
...
def apply_effect(effect, bee, duration):
    ...
    def changed_action(colony):
        ...
        if turns < duration:
            ...
            old_action(colony)
    bee.action = lambda colony: changed_action(colony)
    # END Problem EC
